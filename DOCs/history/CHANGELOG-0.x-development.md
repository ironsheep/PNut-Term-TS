# PNut-Term-TS — 0.x development history (frozen)

**This is the pre-release development log, frozen at v0.11.12 on 2026-07-27, the day
before v1.0.0.** It covers everything from the first commit through the last build made
before the product was published.

- **Frozen — do not edit.** Entries stay exactly as they were written, in the format and
  voice that applied at the time. Do not reformat the headings, do not shorten entries, do
  not apply later rules to it retroactively. The only thing this file is good for is being
  an accurate record of how the work actually went.
- **Not governed by the changelog voicing guide.** That guide governs `CHANGELOG.md` at the
  repository root, which starts at v1.0.0. This file predates it and is exempt.
- **Not maintained.** Nothing is ever added here again. It is written for us, not for users,
  and it is not shipped in the installed packages.
- Its audience was the maintainer and the agents working on the project, so it records root
  causes, rejected approaches, parity findings against the Pascal reference, and internal
  work that a user-facing changelog would exclude.

The format below is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and
the project used [Semantic Versioning](https://semver.org/spec/v2.0.0.html) throughout.
