...
### Fire-and-Forget executeJavaScript() Calls - Potential MaxListeners Warning
**Date Added**: 2025-11-09
**Priority**: Medium - Post-First-Release Cleanup
**Category**: Code Quality / Performance

**Current State**:
Multiple debug windows use fire-and-forget `executeJavaScript()` calls (not awaited, no error handling) that could accumulate `did-stop-loading` listeners under high-frequency rendering scenarios.

**Root Cause**:
Electron's `executeJavaScript()` internally calls `waitTillCanExecuteJavaScript()`, which adds a `once('did-stop-loading')` listener for each call. When calls are made rapidly without awaiting completion, listeners accumulate. The default EventEmitter limit is 20 listeners, triggering MaxListenersExceededWarning at 21.

**Affected Windows**:

1. **debugPlotWin.ts** (HIGH priority - 2 fire-and-forget calls in mouse interaction code)
   - Line 3286: `setupCoordinateDisplayHandler()` - Injects coordinate display code
   - Line 3387: `enableMouseInput()` - Injects mouse input handlers
   - **Impact**: Called during user interaction, could accumulate with frequent mouse activity
   - **Risk**: High-frequency mouse movements could trigger warning

2. **debugFftWin.ts** (HIGH priority - 4 fire-and-forget calls in drawing operations)
   - Line 1579: `drawChannelGrid()` - Draws FFT channel grid
   - Line 2276: `drawLogScaleIndicator()` - Draws log scale indicator
   - Line 2324: `drawChannelLabels()` - Draws channel labels
   - Line 2401: `drawLegend()` - Draws legend
   - **Impact**: Called during spectrum updates, potentially multiple times per second
   - **Risk**: Continuous FFT rendering could accumulate listeners during rapid updates
   - **Note**: All wrapped in try-catch but promises not awaited/handled

3. **debugBitmapWin.ts** (MEDIUM priority - 1 fire-and-forget call in initialization)
   - Line 775: `initializeCanvas()` - Updates window content during canvas setup
   - **Impact**: Called once during window initialization
   - **Risk**: Lower risk due to one-time initialization, but still lacks error handling

4. **debugTermWin.ts** (LOW priority - 1 fire-and-forget call in setup)
   - Line 535: `measureFontMetrics()` - Measures font dimensions for terminal
   - **Impact**: Called once during terminal setup
   - **Risk**: Minimal - single call during initialization

**Example of Problem Pattern**:
```typescript
// UNSAFE (Fire-and-Forget)
this.debugWindow.webContents.executeJavaScript(code);
```

**Recommended Fix Pattern** (minimum - add .catch()):
```typescript
this.debugWindow.webContents.executeJavaScript(code).catch((err) => {
  this.logMessage(`Failed: ${err}`);
});
```

**Best Practice Pattern** (full error handling - debugScopeXyWin.ts model):
```typescript
...
**Evidence**:
- ScopeXY window exhibited MaxListenersExceededWarning during testing with 210 rapid render calls
- Trace warnings confirmed each `executeJavaScript()` adds `did-stop-loading` listener via Electron internals
- ScopeXY was fixed with render throttle pattern (2025-11-09)
- Other windows have same fire-and-forget pattern, could exhibit same issue under heavy use

**Implementation Plan**:
1. Add `.catch()` handlers to all fire-and-forget calls (minimum fix)
2. Consider full `.then().catch()` chains for better observability
3. For high-frequency operations (FFT, Plot), consider throttle pattern like ScopeXY if warnings occur

**Benefits**:
- Prevents MaxListenersExceededWarning under heavy use
- Proper error handling for JavaScript execution failures
- Better observability of rendering issues
- Follows Electron best practices

**Estimated Effort**: 2-3 hours (add error handlers + testing across all affected windows)

**Required for Release**: NO - Not currently exhibiting issues in production, but should be addressed to prevent future warnings


---

### Package Dependency Management - Should Use electron-builder Instead of Manual Copying
**Date Added**: 2025-11-10
**Priority**: Medium - Post-First-Release Refactor
**Category**: Build System / Packaging

**Current State**:
- Custom shell scripts manually copy individual npm modules to packages:
  - `scripts/create-linux-package.sh`
  - `scripts/create-windows-package.sh`
  - `scripts/create-macos-packages.sh`
- Each script has ~30-40 lines copying specific modules (electron, commander, markdown-it, jimp, @jimp/*, serialport, usb, debug, ms, node-gyp-build)
- Architecture-specific native bindings copied manually from `prebuilds/` directory
- Hand-copying approach is brittle and error-prone

**Problems**:
1. **Manual dependency list** - Must update scripts whenever dependencies change
2. **Transitive dependencies missed** - If a module adds new dependencies, scripts don't catch them
3. **Maintenance burden** - Changes must be replicated across 3 platform scripts
4. **Error prone** - Easy to forget a module or get paths wrong
5. **No validation** - Scripts don't verify all dependencies are included
6. **Duplication** - Same logic repeated 3 times (Linux, Windows, macOS)

**Better Approach**:
The project already has **electron-builder** configured (`electron.builder.json` line 4):
```json
"files": ["dist/**/*", "node_modules/**/*", "package.json", "prebuilds/**/*"]
```
...
- ✅ Creates installers/packages for all platforms
- ✅ Handles code signing and notarization
- ✅ Battle-tested (621K weekly downloads)

**Required Changes**:
1. Move `electron` from `devDependencies` to `dependencies` in package.json (it's used at runtime by electron-main.js)
2. Replace custom packaging scripts with electron-builder commands:
   ```json
   "packageLinux": "npm run build && electron-builder build --linux",
   "packageWin": "npm run build && electron-builder build --win",
   "packageMac": "npm run build && electron-builder build --mac"
   ```
3. Configure electron-builder architecture targets if needed
4. Keep custom scripts as backup/reference during transition

**Benefits**:
- ✅ **Automatic dependency resolution** - npm/electron-builder handles it
- ✅ **Industry standard** - Most Electron apps use electron-builder
- ✅ **Single source of truth** - package.json defines what's included
- ✅ **Cross-compilation** - Can build for other platforms from one machine
- ✅ **Less code to maintain** - Delete ~200+ lines of manual copying logic
- ✅ **Auto-updates** - electron-builder has built-in update support
- ✅ **ASAR packaging** - Better performance (already configured: `"asar": true`)

**Risks**:
- ⚠️ **Breaking change** - Packaging output may differ from current custom scripts
- ⚠️ **Testing required** - All platforms need validation after switch
- ⚠️ **Learning curve** - Team needs to understand electron-builder configuration
- ⚠️ **Build time** - electron-builder may be slower than custom scripts

**Why Not Done for v0.9 Release**:
- Too risky to change packaging strategy hours before release
- Custom scripts work correctly (they just need maintenance)
- electron-builder refactor needs thorough testing on all platforms
- Better to ship working product first, optimize build process later

**Estimated Effort**: 4-8 hours (configuration + testing on all platforms)

**Required for Release**: NO - Current manual approach works, refactor for maintainability

**Investigation Questions**:
1. Why were custom scripts created instead of using electron-builder originally?
2. Are there specific requirements the custom scripts fulfill that electron-builder doesn't?
3. Does electron-builder handle the CLI launcher scripts correctly (ELECTRON_RUN_AS_NODE)?

---

### Skipped Debugger Tests - Outdated Design Specifications
**Date Added**: 2025-12-30
**Priority**: Low - Post-Release Cleanup
**Category**: Testing / Code Quality

**Current State**:
Two debugger test files are skipped because they reference modules and methods that don't exist in the current implementation. These tests were written as design specifications before implementation, and the implementation evolved differently.

**Skipped Test Files**:

1. **`tests/debuggerPacketTest.test.ts`** (1 test, entirely skipped)
   - **References non-existent modules**:
     - `CircularBuffer` from `src/classes/shared/circularBuffer.ts`
     - `MessageExtractor`, `MessageType`, `ExtractedMessage` from `src/classes/shared/messageExtractor.ts`
     - `DynamicQueue` from `src/classes/shared/dynamicQueue.ts`
   - **Actual implementation**: Debugger uses `debuggerPhase3Receiver.ts` for packet parsing
   - **Reason**: Architecture changed from planned design - different packet handling approach

2. **`tests/debuggerInteraction.test.ts`** (28 tests, entirely skipped)
   - **References non-existent DebuggerProtocol methods**:
     - `toggleDebugMode()` - not implemented
     - `sendInitCommand()` - not implemented
     - `sendStepCommand()` - not implemented
     - `sendStepOverCommand()` - not implemented
   - **References non-existent DebuggerDataManager methods**:
     - `updateProgramCounter()` - not implemented
   - **Actual implementation**: Protocol has `sendStall()`, `sendBreak()`, `sendGo()`, `requestCogBlock()`, etc.
   - **Reason**: Method signatures and API evolved differently than test specifications anticipated

**Active Debugger Tests** (74 passing):
- `debuggerDataManager.test.ts` - 29 tests ✅
- `debuggerRenderer.test.ts` - 22 tests ✅
- `debuggerProtocol.test.ts` - 23 tests ✅

**Options for Resolution**:
1. **Delete the skipped tests** - They're outdated design documents, not valid tests
2. **Rewrite tests** - Update to match actual implementation signatures
3. **Keep skipped** - Leave as documentation of original design intent

**Recommended Action**:
Delete both skipped test files. They provide no value as tests and serve only as confusing historical artifacts. The active tests provide comprehensive coverage of the actual implementation.

**Estimated Effort**: 30 minutes (delete files, update any references)

**Required for Release**: NO - Tests are already skipped, no functional impact
...
|---------|------------|----------------------------------------------------------------|
| 1.0     | 2025-09-10 | Initial technical debt document                                |
| 1.1     | 2025-10-01 | Added message routing architecture issues                      |
| 1.2     | 2025-10-09 | Added ExtractedMessage interface design review                 |
| 1.3     | 2025-11-04 | Added duplicate mouse input systems                            |
| 1.4     | 2025-11-09 | Added fire-and-forget executeJavaScript() issue                |
| 1.5     | 2025-11-10 | Added package dependency management                            |
| 1.6     | 2025-12-30 | Added skipped debugger tests - outdated design specifications  |
