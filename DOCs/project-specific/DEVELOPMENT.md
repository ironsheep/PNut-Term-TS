...
## 🚨 CRITICAL: Clean Development Philosophy

**FUNDAMENTAL REQUIREMENT**: Production/target machines must remain clean from development environments.

### Core Principles
- **NO development tools on production systems** - All building happens in containers
- **NO npm/node pollution on target machines** - Use pre-built packages only  
- **Container-based development** - All compilation and building in Docker/containers
- **Clean deployment only** - Production machines receive only signed, ready-to-run packages

### Why This Matters
- **Security**: No attack surface from dev tools on production
- **Cleanliness**: No node_modules scattered across filesystem
- **Reproducibility**: Identical builds from containers
- **Maintenance**: Clear separation of dev vs production environments

### Approved Workflow
1. **Develop** in containers (VS Code dev containers, Docker)
2. **Build** everything in containers
3. **Package** self-contained apps in containers
4. **Transfer** only built packages to target
5. **Run** pre-built packages on clean systems

### Example: Electron App Distribution
```bash
# In container - build everything
./scripts/create-electron-ready-package.sh

# On clean Mac - only adds runtime, no dev tools
./SETUP.command  # Just downloads Electron framework
```

This ensures your Mac stays pristine while still allowing full testing!
...
### CRITICAL: Test Execution in Docker Container
This project runs in a Docker container with limited resources. 

**NEVER run `npm test` directly** - it will:
- Spawn parallel test processes
- Saturate container resources  
- Cause container lockup
- Result in complete context loss

**ALWAYS use sequential test execution**:
```bash
# For all tests
scripts/claude/run-all-tests.sh
...
1. IMMEDIATELY add to `scripts/claude/run-all-tests.sh`
2. Add to `scripts/claude/test-runner.sh` if needed
3. Verify the script includes your test:
   ```bash
   grep "yourtest.test.ts" scripts/claude/run-all-tests.sh
   ```
4. Update TEST-STATUS.md with new test

**Before running full test suite**:
1. Audit test scripts for completeness:
   ```bash
   ls tests/*.test.ts | wc -l  # Count actual test files
   grep "test.ts" scripts/claude/run-all-tests.sh | wc -l  # Count in script
   ```
2. These numbers MUST match
