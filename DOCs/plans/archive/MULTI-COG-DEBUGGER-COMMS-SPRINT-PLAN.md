...
**Implemented (2026-07-07, PASM flow):**
- `DAT org 0`, two blocks. Cog 0's **first** instruction is `coginit #1, ##@cog1code`
  (Dest `%0_0_00001` = load-from-hub, specific cog 1; Src = `@cog1code` hub address), then a
  `mov x,#0` / `.loop add x,#1 / jmp #.loop` counter. Cog 1's block (second `org 0`, at hub
  `@cog1code`) is `mov y,#0` / `.cloop add y,#1 / jmp #.cloop`. Both `DEBUG_MAIN=1` +
  `DEBUG_COGINIT=1`.
- **HW step flow:** `DEBUG_MAIN` breaks Cog 0 at `entry` (the coginit). Single-step once →
  the coginit executes → Cog 1 launches → the P2 debug-on-cog-start interrupt fires on Cog 1's
  first instruction → Cog 1 breaks → "Debugger - Cog 1" opens. Both windows then step
  independently (x vs y). A 1-step gate, not the old interpreter walk.
- **KB / source verification:** `COGINIT #1, ##hubaddr` loads 504 longs ($000–$1F7) from hub
  (P2 KB `p2kbPasm2Coginit` + `p2kbPasm2CogHubExecution` `##@label` idiom). `DEBUG_COGINIT`
  compiles to `_brkcond_ = $110` (COGINIT bit `$100` + DEBUG bit `$010`) — a **global** brk
  condition (`p2com.asm:7388`); the debugger ISR breaks on **any** newly-started cog carrying
  the COGINIT brk bit (`Spin2_debugger.spin2:368` `testb brkcz,#23`), so a raw-PASM `coginit`
  breaks exactly as the old Spin2 `cogspin` did (both are the same `coginit` instruction +
  hardware debug-on-cog-start). Listing confirmed the encoding: `coginit` opcode `1100111`,
  Dest=1, Src=$14 (=`@cog1code`), AUGS augment 0.
- **Compile:** `pnut-ts -d` clean → `test12_multicog.bin` 2966 B (was 9254 B as Spin2). Both
  copies updated: root (`.spin2` + `.bin` committed) and `dmg-assets/…` (`.spin2`; that tree
  is gitignored/source-only, no `.bin` for any test). `.lst`/`.map` are not committed artifacts.
- **Assumption for Cert Pass 1 (test FIRST, fail-fast):** that `DEBUG_COGINIT` breaks a
  raw-PASM `coginit`'d cog. Source analysis says yes (hardware debug-on-cog-start is
  Spin/PASM-agnostic); only HW confirms it.
...
- *Streaming classifier — gating confirmed by code review:* every new debug-framing code path
  in `extractionCore.ts` is gated so it is dormant outside a debug session:
  - the per-cog Phase-3 drain branch runs only `if (this.debugPhase3Cog !== null)` — and
    `debugPhase3Cog` is set only when a `DEBUGGERn_416BYTE` Phase-1 is emitted;
  - the Phase-1-before-text reorder (path 1b) runs only `if (this.inDebugSession && …)` — and
    `inDebugSession` flips true only on that same Phase-1 emission;
  - `find416ByteBoundary` (path 3) is unchanged/pre-existing (not added by this sprint).
  So for a pure streaming session (`inDebugSession=false`, `debugPhase3Cog=null`) the
  text/DB/backtick/416 detection order is byte-identical to before.
- *Executable guardrail:* `tests/debuggerStreamingGuardrail.test.ts` — a non-debug stream
  (Cog messages + backtick command) classifies normally and emits ZERO debugger frames, and
  the debug path still engages when a real Phase-1 arrives (the gate is not stuck closed).
- *Streaming/classifier suites green:* `messageClassificationRouting`, `cogMessageRouting`,
  `debugLoggerRouting`, `cog2DebugLoggerRouting` (+ the rest) all pass in the full sequential
  suite alongside the new debug tests.
- *Main-loop responsiveness — code-review confirmation:* the ONLY main-process code added this
  sprint is the §4 `debuggerPhase3Size` relay (`mainWindow` listener → `serialProcessor.
  signalDebuggerPhase3Size`, a pure one-line forward) and the §4 `signalDebuggerReset` forward
  on DTR/RTS. Both are stateless forwards to the worker; NO framing, parsing, buffering, or
  per-break work runs on the Electron main loop. All stateful per-cog framing lives in the
  extraction worker (`ExtractionCore`), preserving the 2 Mbaud responsiveness win from the
  #31 serial offload.
...
Generated by `plan-to-tasks` 2026-07-06. Sprint tag `multicog-comms`. §0 (problem/baseline)
and §1 (architecture) are context, not build deliverables — no task. Cert Pass 1 is an
explicit tracked milestone (the hard gate for §10) with no plan-section number of its own.

| Plan § | Deliverable                                             | Task   | seq |
| ------ | ------------------------------------------------------- | ------ | --- |
| §2     | Worker per-cog demux/framer (CORE)                      | «#66»  | 1   |
| §3     | Renderer Phase-3 size hint; shed cross-break framing    | «#67»  | 2   |
| §4     | Main relay; per-cog isolation & COGBRK verify           | «#68»  | 3   |
| §5     | Phase-2 host→P2 multi-cog-safe regression test          | «#69»  | 4   |
| §6     | test12 rewritten as PASM (HW gate asset)                | «#70»  | 5   |
| §7     | 2-cog synthetic replay fixture + harness tests          | «#71»  | 6   |
| §8     | Non-regression guardrails (classifier & main-loop)      | «#72»  | 7   |
| §9     | ARCHITECTURE.md debugger-routing section                | «#73»  | 8   |
| (gate) | **Cert Pass 1** — HW acceptance gate (gates §10)        | «#74»  | 9   |
| §10    | Remove inert/dead code (post-certification)             | «#75»  | 10  |
| §11    | **Cert Pass 2** — sprint exit (post-cleanup, on HW)     | «#76»  | 11  |
