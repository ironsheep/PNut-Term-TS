...
| --- | --- | --- |
| «#87» «#88» «#89» | ✅ complete | `9da7e46` |
| «#90» «#91» «#92» | ✅ complete | `7a5b0a0` |
| «#93» | ✅ complete — coverage walked against §1–§6, no parallel harness, full sequential sweep green | — |
| «#94» | ✅ complete — `scripts/claude/check_doc_claims.sh` + `check_doc_counts.sh`, `DOC_AUDIT_COMMAND` set | — |
| «#95» | ✅ complete — manual source, Theory §8.3, superseded guide, audit Part B, CHANGELOG, punch list, memory note | — |
| «#96» | ✅ complete — Test Plan **v2**: Phase D (D1–D11) + amended Test 4 steps 8–8c | — |
| «#97» | ⬜ **OPEN — requires hardware.** Tests 0–14 in full plus Phase D. v1.0.1 is not clear to tag until it passes. | — |

Two implementation notes worth carrying forward, both slightly beyond the letter of a
finding and both recorded in the audit's B.3a: the REG/LUT click and hint moved onto the
**inset heat-strip rect** (scaling against the enclosing box would have left a residual
error), and hub data/character clicks now set `DisMode := dmHub` as Pascal does.
