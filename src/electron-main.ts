...
(function main() {
  ...
  if (isDoubleClicked || launchedFromAppBundle) {
    ...
    app.whenReady().then(() => {
      dialog.showMessageBoxSync({
        ...
        detail:
          'PNut-Term-TS is a command-line tool that operates on files in your current directory.\n\n' +
          'To use PNut-Term-TS:\n' +
          '1. Open Terminal\n' +
  ...
  if (global.gc) {
      ...
      for (const entry of entries) {
        if (entry.entryType === 'gc') {
          ...
          if (entry.duration > 10) {
            console.warn(
              `[GC PAUSE] 🔥 ${entry.kind} took ${entry.duration.toFixed(2)}ms - may delay serial receives!`
            );
          }
        }
      }
    ...
    setInterval(() => {
      if (gcStats.count > 0) {
        console.log(
          `[GC STATS] Count: ${gcStats.count}, Total: ${gcStats.totalTime.toFixed(
            0
          )}ms, Max pause: ${gcStats.maxPause.toFixed(2)}ms`
        );
        gcStats.count = 0;
        gcStats.totalTime = 0;
        gcStats.maxPause = 0;
  ...
  electronContext.runEnvironment.exitOnEndSession = config.exitOnEndSession || false;
  electronContext.runEnvironment.headlessEndMarker = config.headlessEndMarker;
  ...
  if (!gotTheLock) {
    logger.warningMsg('Another instance is already running. Exiting.');
    app.quit();
  } else {
    ...
    process.on('SIGUSR1', () => {
      logger.infoMsg('[SIGNAL] Received SIGUSR1 - Resetting hardware via DTR/RTS');
      const mainWindow = (global as any).mainWindowInstance;
      if (mainWindow) {
  ...
  async function createMainWindow() {
    try {
      ...
      if (!electronContext.runEnvironment.quiet) {
        logger.verboseMsg('PNut-Term-TS Electron UI started');
        // logger.logMessage(`Pre-validated configuration received from CLI`);
        if (electronContext.runEnvironment.selectedPropPlug) {
          logger.verboseMsg(`Connected to device: ${electronContext.runEnvironment.selectedPropPlug}`);
        }
      }
